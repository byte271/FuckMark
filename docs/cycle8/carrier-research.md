...
**The search space was empty by construction.** A HarfBuzz advance-and-ink oracle over 286719 assigned code points in all 12 shaping contexts finds 389 code points invisible in every context and 396 invisible in at least one. Every one is `Mn` or `Cf`. The required bundle strips exactly `Mn`, `Cf`, and default-ignorable, so invisibility and required-bundle survival are complementary sets. The intersection is 0 under both definitions, including the loosest one, and no further enumeration can change that.

An earlier revision of this section claimed 12 contexts while the committed scan called the oracle with its default Latin A/B pair only. That was an overclaim, caught in review. The corrected scan iterates all 12; 396 turns out to be the any-context figure, so the original number was right for the loose reading and the correction strengthens the closure rather than weakening it. Errata and per-context counts are in `evidence/h16-local/shaping-closure-12-context.json`. The oracle agrees with real Chromium pixels on 20 of 23 stratified samples; all three disagreements are understood and none weakens the closure.
