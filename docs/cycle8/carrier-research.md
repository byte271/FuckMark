...
**Where the corrected gate stands.** Reading the existing frozen confirmation evidence against `proposed_gate_v2`, with no new corpus and no retuned seed: identity detects 185/192, mix detects 0/192 watermarked and 0/192 unwatermarked, visible text passes 192/192, and every frozen sanitizer still detects 0. Detection after the real sanitizer is `UNMEASURED`, so the proposed gate is **not** claimed satisfied. The remaining work is that one measurement plus a decision about which sanitizers the gate should require — not a further carrier search, which the closure result rules out.
