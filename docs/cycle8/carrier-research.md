...
**Mix still evades the real sanitizer.** Measured on the exploratory lane, seed 890000 with GPT-2 and 48 watermarked rows: pristine detects 47/48, and mix detects 0/48 under raw, NFKC, Cf-strip, and the lm-watermarking `UnicodeSanitizer` alike. The only variant that restores detection is the required bundle, at 47/48 — and it restores the original string on 96/96 rows. That is the mechanism behind the whole H12-H15 blockage: the bundle does not defeat the carrier, it deletes it and hands the detector back the untouched watermarked text. No spent or forbidden seed is used.
