...
FuckMark v0.4.1 is the audit-fix and four-layer mix release on top of the Gate v2 product-authorization CLI. The public CLI algorithm identity is `release-cli-v8`. Do not retag `v0.4.0`. The historical `v0.3.0` tag is the identity CLI (`release-cli-v4`). The historical `v0.2.0` tag still applies contractions.
...
8. Gate v2 still records the historical v1 mix sanitizer gate as FAIL. `required_sanitizers_keep` is not weakened. Do not generate `950000`. Live four-layer mix does not restore under Mn-strip, default-ignorable strip, UnicodeSanitizer orderings, Mn then Me then UnicodeSanitizer, or the required sanitizer bundle. Frozen Cf-strip still removes the Cf layer.
