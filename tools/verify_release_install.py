...
def _verify_artifact(artifact: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="fuckmark-release-") as value:
        ...
        for command in _venv_commands(root):
            ...
            text_flag = _run(
                [str(command), "--text", "I do not agree."],
                capture_output=True,
                env=environment,
            )
            ...
            file_flag = _run(
                [str(command), "--file", str(source_file)],
                capture_output=True,
                env=environment,
            )
            ...
            status_line = _run(
                [str(command), "--text", "I do not agree.", "--status"],
                capture_output=True,
                env=environment,
            )
            if status_line.stdout != expected_arg or "fuckmark-status" not in status_line.stderr:
                raise RuntimeError(
