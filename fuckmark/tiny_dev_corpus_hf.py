...
class HuggingFaceTinyDevBackend:
    def __init__(
        self,
        model_id: str,
        model_revision: str,
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "Install the smoke dependencies before running the real TinyDev corpus generator"
            ) from error
    ...
    def generation_parameters(self, seed: int) -> GenerationParameters:
        return GenerationParameters.create(
            ...
            top_k=self._top_k,
            top_p=self._top_p,
            max_new_tokens=TINY_DEV_TARGET_LENGTH,
            do_sample=True,
            dtype=self._dtype,
            device=self._device,
            backend_id="huggingface-transformers-synthid-tiny-dev",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="fuckmark-tiny-dev-corpus-hf")
    parser.add_argument("--model", default=DEFAULT_MODEL_ID)
    parser.add_argument("--model-revision", default=DEFAULT_MODEL_REVISION)
    parser.add_argument("--device", choices=("auto", "cpu", "cuda"), default="auto")
    parser.add_argument("--seed-base", type=int, default=401000)
