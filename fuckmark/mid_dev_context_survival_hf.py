...
from .experiments.mid_dev_plan_io import (
    load_mid_dev_plan_json,
    load_mid_dev_trace_json,
    validate_mid_dev_plan_trace_binding,
)
from .experiments.mid_dev_scoring import score_mid_dev_frozen_plan
from .hashing import sha256_json
from .mid_dev_context_survival_plan_hf import _write_fsynced
...
MID_DEV_SCORING_PROVENANCE_VERSION = "mid-dev-scoring-provenance-v1"
...
def _validate_plan_provenance(
    ...
    *,
    ...
    fsynced_at = provenance.get("planning_fsynced_at_utc")
    if not isinstance(fsynced_at, str) or not fsynced_at:
        raise ValueError("MidDev plan provenance has no planning fsync timestamp")
    datetime.fromisoformat(fsynced_at)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="fuckmark-mid-dev-context-survival-hf")
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan = load_mid_dev_plan_json(args.plan_json)
    traces = load_mid_dev_trace_json(args.trace_json)
