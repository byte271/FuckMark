...
from ..corpus import CorpusSample, CorpusSplit, TinyDevCorpusArtifact, WatermarkLabel
...
class TinyDevThresholdEvaluation:
    ...
    def __post_init__(self) -> None:
        ...
        threshold_value = _probability("threshold_value", self.threshold_value)
        object.__setattr__(self, "threshold_value", threshold_value)
        achieved = _probability("achieved_calibration_fpr", self.achieved_calibration_fpr)
        ...
        for name, values in (("positive_results", self.positive_results), ("negative_results", self.negative_results)):
            if not isinstance(values, tuple):
                raise TypeError(f"{name} must be a tuple")
            if len(values) != 4:
                raise ValueError(f"{name} must contain four TinyDev attack rows")
            if any(not isinstance(value, CalibratedDetectorResult) for value in values):
                raise TypeError(f"{name} must contain CalibratedDetectorResult values")
            if tuple(sorted(values, key=lambda value: value.sample_id)) != values:
                raise ValueError(f"{name} must use canonical sample ordering")
        all_results = (*self.positive_results, *self.negative_results)
        if len({value.sample_id for value in all_results}) != 8:
            raise ValueError("threshold evaluation must contain eight unique attack samples")
        for value in all_results:
            if (
                value.target_fpr != target
                or value.threshold_hash != self.threshold_hash
                or value.threshold_value != threshold_value
                or value.achieved_calibration_fpr != achieved
                or value.calibration_fpr_interval != self.calibration_fpr_interval
            ):
                raise ValueError("calibrated attack result does not match threshold evaluation")
        ...
        positive_manifest = sha256_json(tuple(value.evidence_hash for value in self.positive_results))
        if self.pristine_baseline.evidence_manifest_hash != positive_manifest:
        ...
        attack_fpr = _probability("attack_negative_fpr", self.attack_negative_fpr)
        if attack_fpr != detected / 4:
        ...
        expected_interval = exact_binomial_interval(detected, 4)
        if self.attack_negative_fpr_interval != expected_interval:
...
class TinyDevDetectorFamilyEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.detector_family not in (DetectorFamily.MEAN, DetectorFamily.WEIGHTED_MEAN):
            raise ValueError("TinyDev v1 detector evidence supports Mean and Weighted Mean only")
        for name, values, expected in (
            ("calibration_evidence", self.calibration_evidence, 100),
            ("attack_evidence", self.attack_evidence, 8),
        ):
            if not isinstance(values, tuple):
                raise TypeError(f"{name} must be a tuple")
            if len(values) != expected:
                raise ValueError(f"{name} must contain exactly {expected} rows")
            if any(not isinstance(value, UncalibratedDetectorEvidence) for value in values):
                raise TypeError(f"{name} must contain UncalibratedDetectorEvidence values")
            if tuple(sorted(values, key=lambda value: value.sample_id)) != values:
                raise ValueError(f"{name} must use canonical sample ordering")
            ...
            if any(value.detector_family is not self.detector_family for value in values):
                raise ValueError(f"{name} detector family does not match container")
        if set(value.sample_id for value in self.calibration_evidence) & set(value.sample_id for value in self.attack_evidence):
            raise ValueError("calibration and attack populations must be disjoint")
        ...
        if self.calibration_bundle.negative_count != 100:
            raise ValueError("calibration bundle must bind 100 negative rows")
        identity = DetectorCalibrationIdentity.from_evidence(self.calibration_evidence[0])
        all_evidence = (*self.calibration_evidence, *self.attack_evidence)
        if self.calibration_bundle.detector_identity != identity:
            raise ValueError("calibration bundle detector identity does not match evidence")
        if any(DetectorCalibrationIdentity.from_evidence(value) != identity for value in all_evidence):
        ...
        if tuple(value.target_fpr for value in self.calibration_bundle.thresholds) != TINY_DEV_HEADLINE_FPRS:
            raise ValueError("calibration bundle must contain frozen 5% and 1% thresholds")
        ...
        if tuple(value.target_fpr for value in self.threshold_evaluations) != TINY_DEV_HEADLINE_FPRS:
            raise ValueError("threshold evaluations must use frozen 5% and 1% order")
        ...
        if self.family_hash != sha256_json(self._payload()):
            raise ValueError("family_hash does not match TinyDev detector family evidence")
...
class TinyDevDetectorEvidenceArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        if self.token_track != TINY_DEV_TOKEN_TRACK or self.prompt_boundary_mode != TINY_DEV_PROMPT_BOUNDARY_MODE:
            raise ValueError("TinyDev detector evidence must use generation tokens and continuation-only boundaries")
        if self.headline_fprs != TINY_DEV_HEADLINE_FPRS or self.primary_fpr != TINY_DEV_PRIMARY_FPR:
            raise ValueError("TinyDev detector FPR profile does not match the frozen 5%/1% design")
        ...
        if not isinstance(self.family_evidence, tuple) or any(
            not isinstance(value, TinyDevDetectorFamilyEvidence) for value in self.family_evidence
        ):
            raise TypeError("family_evidence must contain TinyDevDetectorFamilyEvidence values")
        ...
        for family in self.family_evidence:
            first = family.calibration_evidence[0]
            if (
                first.adapter_id != self.adapter_id
                or first.adapter_algorithm_version != self.adapter_algorithm_version
                or first.adapter_config_hash != self.adapter_config_hash
                or first.source_id != self.adapter_source_id
                or first.source_commit != self.adapter_source_commit
            ):
                raise ValueError("detector family adapter identity does not match top-level artifact")
...
def _populations(artifact: TinyDevCorpusArtifact) -> tuple[tuple[CorpusSample, ...], tuple[CorpusSample, ...]]:
    calibration = tuple(
        sample for sample in artifact.manifest.samples
        if sample.split is CorpusSplit.THRESHOLD_CALIBRATION and sample.label is WatermarkLabel.UNWATERMARKED
    )
    attack = tuple(
        sample for sample in artifact.manifest.samples
    ...
    if len(calibration) != 100 or len(attack) != 8:
        raise TinyDevDetectorEvidenceError("TinyDev corpus does not match the frozen detector populations")
...
def _score_samples(
    ...
    builder: Callable[[NativeObservationBatch], UncalibratedDetectorEvidence],
) -> tuple[UncalibratedDetectorEvidence, ...]:
...
def _evaluate_threshold(
    ...
    attack_evidence: tuple[UncalibratedDetectorEvidence, ...],
    labels: dict[str, WatermarkLabel],
) -> TinyDevThresholdEvaluation:
    calibrated = tuple(sorted(
        (apply_calibration(value, bundle, target_fpr) for value in attack_evidence),
        key=lambda value: value.sample_id,
    ))
    positives = tuple(value for value in calibrated if labels[value.sample_id] is WatermarkLabel.WATERMARKED)
    negatives = tuple(value for value in calibrated if labels[value.sample_id] is WatermarkLabel.UNWATERMARKED)
    ...
    negative_interval = exact_binomial_interval(negative_detected, 4)
    payload = {
        ...
        "attack_negative_fpr": negative_detected / 4,
        "attack_negative_fpr_interval": negative_interval,
    }
    return TinyDevThresholdEvaluation(**payload, evaluation_hash=sha256_json(payload))


def _build_family(
    family: DetectorFamily,
    corpus_id: str,
    calibration_samples: tuple[CorpusSample, ...],
    attack_samples: tuple[CorpusSample, ...],
    adapter: WatermarkAdapter,
    ...
    bundle = calibrate_detector(calibration_rows, scope, target_fprs=TINY_DEV_HEADLINE_FPRS)
    labels = {sample.sample_id: sample.label for sample in attack_samples}
    evaluations = tuple(
        _evaluate_threshold(target, bundle, attack_rows, labels)
...
def build_tiny_dev_detector_evidence(
    ...
    *,
    ...
    if {sample.watermark.watermark_config_hash for sample in artifact.manifest.samples} != {
        expected_watermark_config_hash
    }:
        raise TinyDevDetectorEvidenceError("TinyDev watermark configuration does not match scoring contract")
    ...
    calibration_samples, attack_samples = _populations(artifact)
    families = tuple(
        _build_family(family, artifact.manifest.corpus_id, calibration_samples, attack_samples, adapter)
    ...
    return TinyDevDetectorEvidenceArtifact(**payload, artifact_hash=sha256_json(payload))
...
def primary_baseline_statuses(
    ...
    return tuple(
        (
            ...
            next(
                value.pristine_baseline.status
                for value in family.threshold_evaluations
