...
from .cycle8.compare import CYCLE8_MARGIN_ARM_IDS, CYCLE8_U034F_LETTER_SPACE_ARM_ID
from .cycle8.decision import classify_scale_detector_compare
from .cycle8.ledger import CYCLE8_LETTER_BENCHMARK_PAIR_COUNT, CYCLE8_MARGIN_PRIMARY_SEED_BASE
...
from .seeds.ledger import assert_new_cycle8_margin_generation_seed


CYCLE8_MARGIN_DETECTOR_VERSION = "cycle8-margin-detector-compare-v1"


def run_cycle8_margin_detector_compare(
    *,
    device: str = "cpu",
    seed_base: int = CYCLE8_MARGIN_PRIMARY_SEED_BASE,
    pair_count: int = CYCLE8_LETTER_BENCHMARK_PAIR_COUNT,
    max_attempts: int = 64,
) -> dict[str, object]:
    assert_new_cycle8_margin_generation_seed(seed_base)
    return run_cycle8_detector_compare(
        device=device,
        seed_base=seed_base,
        pair_count=pair_count,
        max_attempts=max_attempts,
        arm_ids=CYCLE8_MARGIN_ARM_IDS,
        sanitizer_ids=CYCLE8_SCALE_SANITIZER_VARIANT_IDS,
        sanitize=sanitize_cycle8_scale_variant,
        algorithm_version=CYCLE8_MARGIN_DETECTOR_VERSION,
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="fuckmark-cycle8-margin-detector")
    parser.add_argument("--device", choices=("cpu", "cuda"), default="cpu")
    parser.add_argument("--seed-base", type=int, default=CYCLE8_MARGIN_PRIMARY_SEED_BASE)
    ...
    assert_new_cycle8_margin_generation_seed(args.seed_base)
    if args.detector_json is None:
        args.detector_json = Path(
            f"evidence/cycle8-margin-{args.seed_base}-n{args.pair_count}-2026-08-26/detector-compare.json"
        )
    detector = run_cycle8_margin_detector_compare(
        device=args.device,
    ...
    decision = classify_scale_detector_compare(
        detector,
        transformed_arm_id=CYCLE8_U034F_LETTER_SPACE_ARM_ID,
    )
