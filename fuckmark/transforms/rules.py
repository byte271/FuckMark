...
GENERAL_WORD_LEADING_SPACING_RULE_VERSION = "general-word-space-before-v1"


@dataclass(frozen=True, slots=True)
class GeneralWordLeadingSpacingRule(LiteralTransformRule):
    def __post_init__(self) -> None:
        require_clean_string("rule_id", self.rule_id)
        require_clean_string("version", self.version)
        if self.version != GENERAL_WORD_LEADING_SPACING_RULE_VERSION:
            raise ValueError("unsupported general word leading spacing rule version")
        if self.family is not TransformFamily.ORTHOGRAPHY:
            raise ValueError("general word leading spacing rules must use the orthography family")
        if self.tier is not TransformTier.SURFACE:
            raise ValueError("general word leading spacing rules must use tier 1 surface")
        if self.source != "word" or self.replacement != "word":
            raise ValueError("general word leading spacing rules use the fixed word sentinel contract")
        if "\n" in self.source or "\r" in self.source or "\n" in self.replacement or "\r" in self.replacement:
            raise ValueError("general word leading spacing rules must stay single-line")
        require_bool("whole_word", self.whole_word)
        require_bool("preserve_simple_case", self.preserve_simple_case)
        require_bool("block_all_caps", self.block_all_caps)
        if self.whole_word or self.preserve_simple_case or self.block_all_caps:
            raise ValueError("general word leading spacing rules use an exact case-sensitive boundary contract")
        require_sha256("rule_hash", self.rule_hash)
        if self.rule_hash != sha256_json(self._payload()):
            raise ValueError("rule_hash does not match general word leading spacing rule")
    ...
    def _payload(self) -> dict[str, object]:
        return {
            ...
            "preserve_simple_case": self.preserve_simple_case,
            "block_all_caps": self.block_all_caps,
            "general_pattern": GENERAL_WORD_LEADING_SPACING_RULE_VERSION,
        }

    @classmethod
    def create(cls, rule_id: str) -> "GeneralWordLeadingSpacingRule":
        hashed = {
            "algorithm_version": RULE_ALGORITHM_VERSION,
            "rule_id": rule_id,
