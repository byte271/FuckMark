...
from .protected_artifacts import InvariantDifference, ProtectedInvariantReport, ProtectedSpanManifest, UserProtectedRange
...
def validate_protected_invariants(
    ...
    original_manifest = extractor.extract(original, user_ranges)
    ...
    user_texts = tuple(sorted({original[value.start:value.end] for value in user_ranges}))
    ...
    differences = tuple(
        InvariantDifference(ProtectedSpanKind(kind), exact_text, original_counter[(kind, exact_text)], transformed_counter[(kind, exact_text)])
