...
SYNTAX_RETOKENIZATION_FIXTURE_ALGORITHM_VERSION = "syntax-retokenization-fixture-v1"


class SyntaxRetokenizationVerificationError(ValueError):
    pass


@dataclass(frozen=True, slots=True)
class SyntaxRetokenizationFixture:
    ...
    def __post_init__(self) -> None:
        ...
        if self.algorithm_version != SYNTAX_RETOKENIZATION_FIXTURE_ALGORITHM_VERSION:
            raise ValueError("unsupported syntax retokenization fixture algorithm version")
        ...
        if self.fixture_hash != sha256_json(self._payload()):
            raise ValueError("fixture_hash does not match syntax retokenization fixture")
...
def _transform_once(rule: SyntaxTemplateRule, source_text: str) -> str:
    if not isinstance(rule, SyntaxTemplateRule):
    ...
    if len(enumeration.candidates) != 1:
        raise ValueError("retokenization fixture source must produce exactly one syntax candidate")
...
def capture_syntax_retokenization_fixture(
    fixture_id: str,
    ...
    payload = {
        "algorithm_version": SYNTAX_RETOKENIZATION_FIXTURE_ALGORITHM_VERSION,
    ...
    return SyntaxRetokenizationFixture(
        algorithm_version=SYNTAX_RETOKENIZATION_FIXTURE_ALGORITHM_VERSION,
        fixture_id=fixture_id,
        rule_hash=rule.rule_hash,
        model_tokenizer_identity_hash=model_tokenizer_identity.identity_hash,
...
def verify_syntax_retokenization_fixture(
    fixture: SyntaxRetokenizationFixture,
    rule: SyntaxTemplateRule,
    model_tokenizer_identity: ModelTokenizerIdentity,
    tokenizer: Callable[[str], Sequence[int]],
) -> None:
    if not isinstance(fixture, SyntaxRetokenizationFixture):
        raise TypeError("fixture must be a SyntaxRetokenizationFixture")
    if not isinstance(rule, SyntaxTemplateRule):
        raise TypeError("rule must be a SyntaxTemplateRule")
    if not isinstance(model_tokenizer_identity, ModelTokenizerIdentity):
        raise TypeError("model_tokenizer_identity must be a ModelTokenizerIdentity")
    if fixture.rule_hash != rule.rule_hash:
        raise SyntaxRetokenizationVerificationError("retokenization fixture rule hash does not match supplied rule")
    if fixture.model_tokenizer_identity_hash != model_tokenizer_identity.identity_hash:
        raise SyntaxRetokenizationVerificationError("retokenization fixture tokenizer identity does not match supplied identity")
    if fixture.transformed_text != _transform_once(rule, fixture.source_text):
        raise SyntaxRetokenizationVerificationError("retokenization fixture transformed text does not replay from supplied rule")
    if fixture.source_token_ids != _tokenize(tokenizer, fixture.source_text):
        raise SyntaxRetokenizationVerificationError("retokenization fixture source tokens do not replay from supplied tokenizer")
    if fixture.transformed_token_ids != _tokenize(tokenizer, fixture.transformed_text):
        raise SyntaxRetokenizationVerificationError("retokenization fixture transformed tokens do not replay from supplied tokenizer")
