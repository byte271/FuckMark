...
The CLI loads packaged copies under `fuckmark/robustness_data/` that must
match these `specs/` files byte-for-byte. Before reporting success it
recomputes protocol, vectors-file, vectors-canonical, and scorecard-file
SHA-256 and compares them to this freeze record. Changing scorecard bytes
while leaving the embedded `scorecard_hash` field alone is still a mismatch.

## 6. Exit status

`fuckmark robustness` exits 0 when every selected cell matches the frozen
vectors, the freeze hashes still match the loaded artifacts, and the sealed
scorecard hash still matches. Exit 1 is a mismatch (mix, sanitizer, or
freeze-hash drift). Exit 2 is usage.
