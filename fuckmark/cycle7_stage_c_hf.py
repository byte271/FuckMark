...
CYCLE7_STAGE_C_DETECTOR_VERSION = "cycle7-stage-c-detector-compare-v1"
    ...
    *,
    ...
    if samples_from is not None:
        ...
        if int(previous["seed_base"]) != seed_base:
            raise ValueError("Stage C sample file seed_base does not match the requested ledger seed")
    else:
        samples = _generate_stage_b_samples(backend, seed_base, topic, "cycle7-stage-c1")
    ...
    if not skip_detector:
        ...
        detector = {
            **{k: v for k, v in detector.items() if k != "artifact_hash"},
            "algorithm_version": CYCLE7_STAGE_C_DETECTOR_VERSION,
            "seed_base": seed_base,
            "stage": "C1",
            "topic": topic,
        }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="fuckmark-cycle7-stage-c")
    parser.add_argument("--device", choices=("cpu", "cuda"), default="cpu")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("evidence/cycle7-stage-c-2026-08-25"),
    )
    ...
    if args.validation:
        raise RuntimeError(
            f"Stage C validation seed {CYCLE7_STAGE_C_VALIDATION_SEED_BASE} is reserved until a catalog freeze"
        )
    ...
    sample_payload = {
        "algorithm_version": "cycle7-stage-c-samples-v1",
