...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="fuckmark-tiny-dev-transform-extended-hf")
    ...
    parser.add_argument("--plan-json", type=Path, default=Path("artifacts/tiny-dev-transform-extended-plan.json"))
    parser.add_argument("--json", type=Path, default=Path("artifacts/tiny-dev-transform-extended-evidence.json"))
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not getattr(tokenizer, "is_fast", False):
        raise RuntimeError("TinyDev extended transform geometry requires a fast tokenizer with offset mappings")
    ...
    plan = build_transform_plan(
        corpus,
        tokenizer,
        budgets=EXTENDED_TINY_DEV_BUDGETS,
    ...
    sys.stdout.write(f"budgets={','.join(str(value) for value in EXTENDED_TINY_DEV_BUDGETS)}\n")
    sys.stdout.write(
