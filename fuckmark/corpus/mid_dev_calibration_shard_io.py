...
def _parse_json(text: str):
    try:
        ...
        raise TinyDevCorpusJsonError("calibration shard JSON is not valid JSON") from error
...
def _output_manifest(value: object) -> MidDevCalibrationShardOutputManifest:
    data = _mapping(
        ...
        value,
        (
            "algorithm_version", "role", "plan_hash", "shard_id", "shard_spec_hash",
            "target_length", "source_indices", "prompt_ids", "sample_ids", "sample_record_hashes",
            "text_sha256s", "continuation_token_hashes", "model_tokenizer_identity_hash",
            "watermark_config_hash", "watermark_condition_hash", "output_hash",
        ),
    )
    return MidDevCalibrationShardOutputManifest(
        algorithm_version=data["algorithm_version"], role=CalibrationRole(data["role"]),
        plan_hash=data["plan_hash"], shard_id=data["shard_id"], shard_spec_hash=data["shard_spec_hash"],
        target_length=data["target_length"], source_indices=_tuple_ints("source_indices", data["source_indices"]),
        ...
        watermark_config_hash=data["watermark_config_hash"], watermark_condition_hash=data["watermark_condition_hash"],
        output_hash=data["output_hash"],
    )


def parse_mid_dev_generated_calibration_shard_json(text: str, *, require_canonical: bool = True) -> MidDevGeneratedCalibrationShard:
    ...
    if require_canonical and text not in (canonical_json_text(shard), canonical_json_text(shard) + "\n"):
...
def _merged_manifest(value: object) -> MidDevCalibrationMergedManifest:
    data = _mapping(
        ...
        value,
        (
            "algorithm_version", "role", "plan_hash", "prompt_manifest_hash", "shard_output_hashes",
            "prompt_ids", "sample_ids", "sample_record_hashes", "text_sha256s", "continuation_token_hashes",
            "model_tokenizer_identity_hash", "watermark_config_hash", "watermark_condition_hash", "manifest_hash",
        ),
    )
    return MidDevCalibrationMergedManifest(
        algorithm_version=data["algorithm_version"], role=CalibrationRole(data["role"]), plan_hash=data["plan_hash"],
        ...
        prompt_ids=_tuple_strings("prompt_ids", data["prompt_ids"]), sample_ids=_tuple_strings("sample_ids", data["sample_ids"]),
        sample_record_hashes=_tuple_strings("sample_record_hashes", data["sample_record_hashes"]),
        text_sha256s=_tuple_strings("text_sha256s", data["text_sha256s"]),
        continuation_token_hashes=_tuple_strings("continuation_token_hashes", data["continuation_token_hashes"]),
        model_tokenizer_identity_hash=data["model_tokenizer_identity_hash"],
...
def parse_mid_dev_calibration_merged_manifest_json(text: str, *, require_canonical: bool = True) -> MidDevCalibrationMergedManifest:
    try:
        manifest = _merged_manifest(_parse_json(text))
    except Exception as error:
        if isinstance(error, TinyDevCorpusJsonError):
            raise
        raise TinyDevCorpusJsonError("calibration merged manifest failed validation") from error
    if require_canonical and text not in (canonical_json_text(manifest), canonical_json_text(manifest) + "\n"):
