...
MID_DEV_CORPUS_ALGORITHM_VERSION = "mid-dev-context-survival-corpus-v2"
MID_DEV_ANALYSIS_SPLIT_VERSION = "mid-dev-source-holdout-v1"
MID_DEV_TARGET_LENGTHS = (128, 256)
MID_DEV_SOURCE_COUNT = 36
MID_DEV_SOURCES_PER_LENGTH = 18
MID_DEV_SOURCES_PER_FAMILY_LENGTH = 3
MID_DEV_FIT_SOURCES_PER_FAMILY_LENGTH = 2
MID_DEV_EVAL_SOURCES_PER_FAMILY_LENGTH = 1
MID_DEV_SEED_BASE = 510_000
MID_DEV_PROMPT_SOURCE_ID = "fuckmark-mid-dev-context-survival-prompts-v2"
MID_DEV_PROMPT_LICENSE_ID = "LicenseRef-FuckMark-Unspecified"
MID_DEV_PROMPT_PROVENANCE = "fuckmark/corpus/mid_dev.py"


class MidDevAnalysisSplit(str, Enum):
    FIT = "analysis_fit"
...
def build_mid_dev_prompt_records() -> tuple[PromptRecord, ...]:
    expected_topic_count = (
        len(MID_DEV_PROMPT_FAMILIES)
        * len(MID_DEV_TARGET_LENGTHS)
...
def mid_dev_seed_for_prompt(prompt_id: str) -> int:
    require_clean_string("prompt_id", prompt_id)
    prompt_ids = tuple(value.prompt_id for value in build_mid_dev_prompt_records())
    try:
        index = prompt_ids.index(prompt_id)
    except ValueError as error:
        raise MidDevCorpusError("prompt_id is not part of the frozen MidDev matrix") from error
    return MID_DEV_SEED_BASE + index


def build_mid_dev_analysis_split(
    prompts: Sequence[PromptRecord] | None = None,
) -> dict[str, MidDevAnalysisSplit]:
    materialized = tuple(prompts) if prompts is not None else build_mid_dev_prompt_records()
    if len(materialized) != MID_DEV_SOURCE_COUNT:
        raise MidDevCorpusError("analysis split requires the complete frozen MidDev prompt matrix")
    cells: dict[tuple[str, int], list[PromptRecord]] = {}
    for prompt in materialized:
        key = (prompt.prompt_family_id, mid_dev_target_length_for_prompt(prompt.prompt_id))
        cells.setdefault(key, []).append(prompt)
    expected_cells = len(MID_DEV_PROMPT_FAMILIES) * len(MID_DEV_TARGET_LENGTHS)
    if len(cells) != expected_cells:
        raise MidDevCorpusError("analysis split is missing a prompt-family/length cell")
    output: dict[str, MidDevAnalysisSplit] = {}
    for key in sorted(cells):
        values = cells[key]
        if len(values) != MID_DEV_SOURCES_PER_FAMILY_LENGTH:
            raise MidDevCorpusError("analysis split requires exactly three sources in every cell")
        ranked = sorted(
            values,
            key=lambda value: (
                sha256_json(
                    {
                        "algorithm_version": MID_DEV_ANALYSIS_SPLIT_VERSION,
        ...
        for prompt in ranked[:MID_DEV_FIT_SOURCES_PER_FAMILY_LENGTH]:
            output[prompt.prompt_id] = MidDevAnalysisSplit.FIT
        for prompt in ranked[MID_DEV_FIT_SOURCES_PER_FAMILY_LENGTH:]:
            output[prompt.prompt_id] = MidDevAnalysisSplit.EVALUATION
    if Counter(output.values()) != Counter(
        ...
        raise MidDevCorpusError("analysis split must contain 24 fit and 12 evaluation sources")
    return output


def mid_dev_analysis_split_hash(
    prompts: Sequence[PromptRecord] | None = None,
) -> str:
    ...
    return sha256_json(
        {
            "algorithm_version": MID_DEV_ANALYSIS_SPLIT_VERSION,
            "assignments": tuple(
                (prompt_id, split[prompt_id].value)
                for prompt_id in sorted(split)
            ),
        }
    )
...
class MidDevAttackArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        require_sha256("analysis_split_hash", self.analysis_split_hash)
        require_sha256("artifact_hash", self.artifact_hash)
        _validate_mid_dev_manifest(self.manifest)
        if self.analysis_split_hash != mid_dev_analysis_split_hash(self.manifest.prompts):
            raise ValueError("analysis_split_hash does not match MidDev prompt assignments")
...
def _validate_mid_dev_manifest(manifest: CorpusManifest) -> None:
    ...
    if len(prompts) != MID_DEV_SOURCE_COUNT:
        raise MidDevCorpusError(
            f"MidDev must contain exactly {MID_DEV_SOURCE_COUNT} independent prompts"
        )
    ...
    if dict(length_counts) != {
        value: MID_DEV_SOURCES_PER_LENGTH for value in MID_DEV_TARGET_LENGTHS
    }:
    ...
    if dict(family_length_counts) != expected_family_length_counts:
        raise MidDevCorpusError(
            "MidDev source counts must be balanced across prompt-family and length cells"
        )
    ...
    for sample in samples:
        ...
        expected_seed = mid_dev_seed_for_prompt(prompt.prompt_id)
        if sample.generation.seed != expected_seed:
            raise MidDevCorpusError("MidDev sample generation seed does not match its frozen source seed")
    for values in by_match.values():
        if len(values) != 2:
            raise MidDevCorpusError("each MidDev source group must contain exactly two samples")
        if {value.label for value in values} != {
            WatermarkLabel.WATERMARKED,
            WatermarkLabel.UNWATERMARKED,
        }:
            raise MidDevCorpusError(
                "each MidDev source group must contain one watermarked and one control sample"
            )
...
def build_mid_dev_attack_artifact(
    ...
    split_hash = mid_dev_analysis_split_hash(manifest.prompts)
    payload = {
        "algorithm_version": MID_DEV_CORPUS_ALGORITHM_VERSION,
        ...
        "analysis_split_hash": split_hash,
    }
