...
- Recorded `cycle8-mix-publishability-v1` after merging that freeze onto `main`. Product hardening on that report: software compatibility PASS on the UTF-8 product surface (Latin-1/ASCII/cp1252 unsupported; visible-projection search; raw codepoint search is not the product API). Sanitizer weaknesses still FAIL; assigned-Unicode feasibility found no stronger Priority-Zero invisible mechanism. Cross-detector still FAIL; Mean versus Weighted Mean on the same GPT-2 Hugging Face SynthID adapter is HYPOTHESIS mix **0/192**, not a second model and not a confirmation rewrite. Mix is not product-publishable. Tag `v0.3.0` is not retagged.
