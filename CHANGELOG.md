...
- The public command now transforms ordinary English ASCII text. Visible words stay exactly the same. Running `fuckmark` in a terminal opens a paste UI: finish with a line that is only `:done`, copy the payload, and do not print hidden Unicode. Pipes, quoted text, files, and `--stdin` stay script-friendly on stdout. `--visible` prints the original visible text. `--copy` copies stream output (UTF-16 on Windows `clip`, UTF-8 elsewhere). Unsupported Unicode is returned unchanged. Invalid UTF-8 and empty input fail closed with an actionable error.
