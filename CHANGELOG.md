...
- Static npm site in `web/`: Vite build of the product engine (insert, detect, strip, hard-machine spans) for GitHub Pages / Netlify / any static host. `npm test` in `web/` checks TypeScript output against Python goldens. HuggingFace / SynthID research remains Python-only.
