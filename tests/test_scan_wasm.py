...
(async () => {
  ...
  if (scanned.findings[0].index !== 0) throw new Error("missing index " + scanned.findings[0].index);
  if (scanned.truncated !== false) throw new Error("truncated " + scanned.truncated);
  if (!scanned.highest_severity) throw new Error("missing highest_severity");
  if (scanned.source_length !== 1) throw new Error("source_length " + scanned.source_length);
  const mixed = api.scanText("a\uD800b", null, "auto");
  if (mixed.findings[0].index !== 1) throw new Error("mixed index " + mixed.findings[0].index);
  if (mixed.source_length !== 3) throw new Error("mixed source_length " + mixed.source_length);
