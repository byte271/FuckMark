...
PERMITTED_HISTORICAL_MIX_DRIFT = {
    "cycle8-1080000-51-structured_instructional-unwatermarked": {
        ...
        "historical_mix_sha256": "f6d4464ebbbf3adeae5b118e80151d38ad8e7511f47dc55aa309fd938a6fb860",
    },
    "cycle8-1080000-55-structured_instructional-unwatermarked": {
        ...
        "historical_mix_sha256": "e5c6da6a45f00a3b614752bef46c852e2a9ae2384d06fa83d4d679cddd1ab660",
    },
    "cycle8-840000-13-technical_explanation-watermarked": {
        ...
        "historical_mix_sha256": "845ec79d12040a9a77505d7da8b78b6de3e5c8ab94f9dc3cfc5ae8f6222642b9",
    },
}


def historical_mix_hash(source: str) -> str:
    mixed = apply_historical_mark_letter_mix(source)
    assert project_visible_v1(mixed, HISTORICAL_MARK_MIX_CARRIERS) == source
    if mixed != source:
        assert is_carrier_insertion_v1(source, mixed, HISTORICAL_MARK_MIX_CARRIERS)
    return sha256_text(mixed)


def assert_live_dual_layer_resists_stress(source: str) -> str:
...
def assert_live_mix_matches_stored(sample_id: str, source: str, stored_hash: str, *, label: str) -> str:
    historical = historical_mix_hash(source)
    assert_live_dual_layer_resists_stress(source)
    if historical == stored_hash:
        return historical
    allowed = PERMITTED_HISTORICAL_MIX_DRIFT.get(sample_id)
    if allowed is None:
        raise AssertionError(f"unexplained historical mix drift for {sample_id}")
    ...
    expected = allowed.get("historical_mix_sha256")
    if expected is not None and historical != expected:
        raise AssertionError(f"permitted historical drift hash changed for {sample_id}")
    return historical
