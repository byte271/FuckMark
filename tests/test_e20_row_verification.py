...
def _fixture():
    ...
    serialized = {
        entry.entry_hash: secret_by_key_id[entry.key_id]
        for entry in key_manifest.entries
    }
    ...
    authorization = authorize_e20_execution(
        ...
        worker_version="e20-row-test-worker-v1",
        shard_count=4,
    ...
    source_sample = next(
        ...
        if value.label is WatermarkLabel.WATERMARKED and value.watermark.key_id == "test-key-0"
    )
    ...
    seed = derive_e20_condition_seed(
        authorization,
        corpus_manifest,
    ...
    transformed_batch = build_native_observations(
        f"{source_sample.sample_id}:{condition.condition_id}:transformed",
    ...
    artifacts = {
        ...
        "schedule_input": scheduler_input,
        ...
        "original_detector_result": original_detector_result,
        "transformed_detector_result": transformed_detector_result,
...
def test_source_replay_rejects_schedule_seed_not_derived_from_sealed_execution() -> None:
    ...
    with pytest.raises(E20RowVerificationError, match="deterministic seed derivation"):
        build_e20_outcome_row(**changed)


def test_source_replay_rejects_transformed_batch_without_condition_bound_identity() -> None:
    artifacts = _fixture()
    wrong_batch = build_native_observations(
        "wrong-transformed-id",
        artifacts["transformed_tokens"].token_ids,
        artifacts["source_sample"].model.eos_token_id,
        artifacts["adapter"],
    )
    changed = dict(artifacts)
