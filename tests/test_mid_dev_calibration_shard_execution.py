...
def test_frozen_readiness_shard_ids_cover_exact_500_ranges():
    readiness = FROZEN_MID_DEV_CALIBRATION_READINESS_PLAN
    for plan in (readiness.select_plan, readiness.audit_plan):
        assert len(plan.shards) == 80
        for shard in plan.shards:
            assert shard.end_index_exclusive - shard.start_index == 500
            assert len(shard.prompt_ids) == 500
            assert len(shard.seeds) == 500
