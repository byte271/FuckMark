...
Cycle 8 U+034F x1 is a `PROMISING_DEVELOPMENT` / `HYPOTHESIS`. Tiny 4-pair corpora are 0/12 transformed WM. Scale exploratory seed `930000` is 0/16, 0/32, then **1/64** raw transformed WM. Independent replication seed `940000` is **0/64**. Combined large-N on those two 64-pair corpora is **1/128**. Visible projection passed. This is not product-authorized and is not a 0/192 claim. Seed `960000` is reserved for detector-blind density follow-up and is not generated in the reservation commit. Seed `880000` is publicly exposed by closed unmerged PR #98 and is not eligible as unseen validation. See [`docs/seeds.md`](docs/seeds.md).
